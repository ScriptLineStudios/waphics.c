...
void player_move(Surface display, Player *player, Tilemap *tilemap) {
...
void player_update(Surface display, Player *player, Tilemap *tilemap) {
    waphics_draw_rect(display, RECT(player->rect.x - player->camera[0], player->rect.y - player->camera[1], player->rect.w, player->rect.h), player->color);
    ...
    waphics_draw_image(display, RECT(0, 0, block_width, block_height), 3, block_pixels);
    player->movement[1]+=4;
    // for (int i = 0; i < tilemap->num_tiles; i++) {
        // Rectangle tile = tilemap->tiles[i];
        // waphics_draw_image(display, RECT(tile.x - player->camera[0], tile.y - player->camera[1], block_width, block_height), 3, block_pixels);
        // waphics_draw_rect(display, RECT(tile.x - player->camera[0], tile.y - player->camera[1], tile.w, tile.h), 0xFFFF0000);
    // }

    player_move(display, player, tilemap);
}